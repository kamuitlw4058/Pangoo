...
namespace NPinyin
{
    public static class Pinyin
    {
        ...
        public static string GetPinyin(string text, bool camelCase = true)
        {
            StringBuilder sbPinyin = new StringBuilder();
            for (var i = 0; i < text.Length; ++i)
            {
                string py = GetPinyin(text[i], camelCase);
        ...
        public static string GetPinyin(char ch, bool camelCase = true)
        {
            ...
            for (var i = 0; i < PyHash.hashes[hash].Length; ++i)
            {
                ...
                if (pos != -1)
                {
                    if (camelCase)
                    {
                        return PyCode.codes[index].Substring(0, 6).Trim().ToPascal();
                    }
                    else
                    {

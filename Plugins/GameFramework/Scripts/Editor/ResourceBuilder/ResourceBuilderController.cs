...
namespace UnityGameFramework.Editor.ResourceTools
{
    public sealed partial class ResourceBuilderController
    {
        ...
        private bool BuildResources(Platform platform, AssetBundleBuild[] assetBundleBuildDatas, BuildAssetBundleOptions buildAssetBundleOptions, ResourceData[] assetBundleResourceDatas, ResourceData[] binaryResourceDatas)
        {
            ...
            Debug.Log($"Unity start build asset bundles for '{platformName}'...");
            ...
            {
                foreach (var ab in assetBundleBuildDatas)
                {
                    foreach (var asset in ab.assetNames)
                    {
                        m_BuildReport.LogError($"Build asset nams for '{platformName}' :{ab.assetBundleName} -{asset}.");
                    }

                }
                m_BuildReport.LogError($"Build asset bundles for '{platformName}' failure.workingPath:{workingPath} assetBundleBuildDatas:{assetBundleBuildDatas.Length}, buildAssetBundleOptions:{buildAssetBundleOptions}, targert:{GetBuildTarget(platform)}");
                m_BuildReport.LogError("Build asset bundles for '{0}' failure.", platformName);
                Debug.Log($"Build asset bundles for '{platformName}' failure");

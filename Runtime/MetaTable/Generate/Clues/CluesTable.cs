...
{
    [Serializable]
    public partial class CluesTable : MetaTableBase,ICluesTable
    {


        public Dictionary<string, ICluesRow> RowDict = new Dictionary<string, ICluesRow>();
               ...
               if (RowDict.ContainsKey(o.Uuid))
                {
                   ...
                   RowDict.Add(o.Uuid, o as ICluesRow);
            }
        }
        ...
        public override void MergeRows(IReadOnlyList<IMetaTableRow> rows)
        {
            for (int i = 0; i < rows.Count; i++)
            {
                ...
                {
                    RowDict.Add(rows[i].Uuid, rows[i] as ICluesRow);
                }
            }
        }

        public ICluesRow GetRowByUuid(string uuid)
        {
            if (RowDict.TryGetValue(uuid, out ICluesRow row))
            {
                return row;
            }
            return null;
        ...
        public ICluesRow GetRowById(int id)
        {

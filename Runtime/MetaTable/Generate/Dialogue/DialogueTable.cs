...
{
    [Serializable]
    public partial class DialogueTable : MetaTableBase,IDialogueTable
    {


        public Dictionary<string, IDialogueRow> RowDict = new Dictionary<string, IDialogueRow>();
               ...
               if (RowDict.ContainsKey(o.Uuid))
                {
                   ...
                   RowDict.Add(o.Uuid, o as IDialogueRow);
            }
        }
        ...
        public override void MergeRows(IReadOnlyList<IMetaTableRow> rows)
        {
            for (int i = 0; i < rows.Count; i++)
            {
                ...
                {
                    RowDict.Add(rows[i].Uuid, rows[i] as IDialogueRow);
                }
            }
        }

        public IDialogueRow GetRowByUuid(string uuid)
        {
            if (RowDict.TryGetValue(uuid, out IDialogueRow row))
            {
                return row;
            }
            return null;
        ...
        public IDialogueRow GetRowById(int id)
        {

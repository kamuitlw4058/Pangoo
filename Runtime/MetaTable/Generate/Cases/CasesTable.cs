...
{
    [Serializable]
    public partial class CasesTable : MetaTableBase,ICasesTable
    {


        public Dictionary<string, ICasesRow> RowDict = new Dictionary<string, ICasesRow>();
               ...
               if (RowDict.ContainsKey(o.Uuid))
                {
                   ...
                   RowDict.Add(o.Uuid, o as ICasesRow);
            }
        }
        ...
        public override void MergeRows(IReadOnlyList<IMetaTableRow> rows)
        {
            for (int i = 0; i < rows.Count; i++)
            {
                ...
                {
                    RowDict.Add(rows[i].Uuid, rows[i] as ICasesRow);
                }
            }
        }

        public ICasesRow GetRowByUuid(string uuid)
        {
            if (RowDict.TryGetValue(uuid, out ICasesRow row))
            {
                return row;
            }
            return null;
        ...
        public ICasesRow GetRowById(int id)
        {

...
{
    [Serializable]
    public partial class ActorsLinesTable : MetaTableBase,IActorsLinesTable
    {


        public Dictionary<string, IActorsLinesRow> RowDict = new Dictionary<string, IActorsLinesRow>();
               ...
               if (RowDict.ContainsKey(o.Uuid))
                {
                   ...
                   RowDict.Add(o.Uuid, o as IActorsLinesRow);
            }
        }
        ...
        public override void MergeRows(IReadOnlyList<IMetaTableRow> rows)
        {
            for (int i = 0; i < rows.Count; i++)
            {
                ...
                {
                    RowDict.Add(rows[i].Uuid, rows[i] as IActorsLinesRow);
                }
            }
        }

        public IActorsLinesRow GetRowByUuid(string uuid)
        {
            if (RowDict.TryGetValue(uuid, out IActorsLinesRow row))
            {
                return row;
            }
            return null;
        ...
        public IActorsLinesRow GetRowById(int id)
        {

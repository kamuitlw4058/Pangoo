...
{
    [Serializable]
    public partial class ClueTable : MetaTableBase,IClueTable
    {


        public Dictionary<string, IClueRow> RowDict = new Dictionary<string, IClueRow>();
               ...
               if (RowDict.ContainsKey(o.Uuid))
                {
                   ...
                   RowDict.Add(o.Uuid, o as IClueRow);
            }
        }
        ...
        public override void MergeRows(IReadOnlyList<IMetaTableRow> rows)
        {
            for (int i = 0; i < rows.Count; i++)
            {
                ...
                {
                    RowDict.Add(rows[i].Uuid, rows[i] as IClueRow);
                }
            }
        }

        public IClueRow GetRowByUuid(string uuid)
        {
            if (RowDict.TryGetValue(uuid, out IClueRow row))
            {
                return row;
            }
            return null;
        ...
        public IClueRow GetRowById(int id)
        {

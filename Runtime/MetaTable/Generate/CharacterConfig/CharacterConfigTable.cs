...
{
    [Serializable]
    public partial class CharacterConfigTable : MetaTableBase,ICharacterConfigTable
    {


        public Dictionary<string, ICharacterConfigRow> RowDict = new Dictionary<string, ICharacterConfigRow>();
               ...
               if (RowDict.ContainsKey(o.Uuid))
                {
                   ...
                   RowDict.Add(o.Uuid, o as ICharacterConfigRow);
            }
        }
        ...
        public override void MergeRows(IReadOnlyList<IMetaTableRow> rows)
        {
            for (int i = 0; i < rows.Count; i++)
            {
                ...
                {
                    RowDict.Add(rows[i].Uuid, rows[i] as ICharacterConfigRow);
                }
            }
        }

        public ICharacterConfigRow GetRowByUuid(string uuid)
        {
            if (RowDict.TryGetValue(uuid, out ICharacterConfigRow row))
            {
                return row;
            }
            return null;
        ...
        public ICharacterConfigRow GetRowById(int id)
        {
